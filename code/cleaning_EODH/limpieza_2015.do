********************************************************
* version 3:
* data cleaning for the 2015 mobility survey
********************************************************
...
*******************************************************************************************
// TOTAL NUMER OF PEOPLE IN THE HOUSEHOLD
******************************************************************************************* 
...
********************************************************
// viajes module describes person's trips
*******************************************************
...
/*
I changed the travel purpose categories as follows:

1.  Categories 7 ("Buscar/dejar a alguien") and 8 ("Buscar/dejar algo") are collapsed
    into a single category representing picking up or dropping off someone.
    Specifically, original category 8 is reassigned to 7.

2.  To preserve consecutive coding after the collapse, all subsequent categories
    are shifted down by one:
        - 9  → 8   ("Comer/tomar algo")
        - 10 → 9   ("Compras")
        - 11 → 10  ("Trámites")
        - 12 → 11  ("Recreación")
        - 13 → 12  ("Buscar trabajo")
        - 14 → 13  (next category in original coding)

3.  Code 89 is recoded to 77 ("Otra cosa").
...
*******************************************************************************************
// PERSONAS_ANONIMIZADO indicates de socioeconomic caracteristics of each person
*******************************************************************************************
...
* this variable needs to be treated carefully, because it must be constructed with two variables from the survey 
gen ocupacion1=.

// is obrero
replace ocupacion1=1 if id_trabajoactividad==1

// is empleado domestico
replace ocupacion1=4 if id_trabajoactividad==3

// is Trabajador independiente o por cuenta propia 
replace ocupacion1=5 if id_trabajoactividad==5

// is 04. Profesional independiente 
replace ocupacion1=6 if id_trabajoactividad==4

// is Patrón o empleador 
replace ocupacion1=7 if id_trabajoactividad==6

// i merged 08. Trabajador sin remuneración  09. Trabajador familiar sin remuneración (hijo o familiar de empleado doméstico, mayordomo, jornalero, etc.) into 8 "Trabajo familiar (sin remuneración)"
replace ocupacion1=8 if id_trabajoactividad==8 | id_trabajoactividad==9

// is empleado publico
replace ocupacion1=27 if id_trabajoactividad==2

//empresa particular (this one make no sense, but the survey says 01. Obrero o empleado de empresa particular)
replace ocupacion1=28 if id_trabajoactividad==1 & ocupacion1 == .

// I merged 10. Jornalero o peón  07. Trabajador de finca, tierra o parcela, propia, en arriendo, aparcería, usufructo o posesión.  into Jornalero/agricultor
replace ocupacion1=29 if id_trabajoactividad==10 | id_trabajoactividad==7

// if the occupation is still not established, leave trabaja desde la casa if it is equal to 3
// this is tricky because a person can both have a certain occupation and work at home
replace ocupacion1=9 if trabajo_casa==3 & ocupacion1 == .

//is estudiante
replace ocupacion1=13 if id_actividad==3 & ocupacion1 == .

//is dedicado al hogar
